...
class OperationsTest {
    ...
    fun `test add`() = runBlocking {
        ...
        val userId = UserId(add(users, user))
        assertEquals(
            expected = user,
            actual = get(users / userId)
        )
        val message = Message()
        val messageId = MessageId(add(users / userId / messages, message))
        assertEquals(
            expected = message,
    ...
    fun `test put`() = runBlocking {
        val user = User().also(usersCreated::add)
        put(users / UserId(user.id), user)
        assertEquals(
            expected = user,
            actual = get(users / UserId(user.id))
        )
        val message = Message()
        put(users / UserId(user.id) / messages / MessageId(message.id), message)
        assertEquals(
            expected = message,
            actual = get(users / UserId(user.id) / messages / MessageId(message.id))
        )
    }
    ...
    fun `test getAll`() = runBlocking {
        ...
        assertEquals(
            ...
            actual = getAll(users).toSet()
        )

        assertEquals(
            expected = messagesCreated,
            actual = getAll(users / UserId(user.id) / messages).toSet()
        )
    }
    ...
    fun `test delete collection`() = runBlocking {
        ...
        repeat(7) {
            ...
            put(users / UserId(user.id), user)
            repeat(7) {
                add(users / UserId(user.id) / messages, Message())
            }
        }
        ...
        assertTrue(getAll<Message>(users / UserId(user.id) / messages).isNotEmpty())
        deleteAll(users / UserId(user.id) / messages)
        assertTrue(getAll<Message>(users / UserId(user.id) / messages).isEmpty())
